...
class CorrTX:
    def __init__(self, pid, endian='>',ip='10.0.0.1', x_per_fpga=2, port=7147, payload_len=4096, verbose=False, timestamp_rnd=1024*128, store_in_cache=False):
        self.pid=pid
        self.endian = endian
...
if __name__ == '__main__':
    ...
    if len(args) < 1: 
        ...
        sys.exit()
    pid =  int(args[0])
    c = CorrTX(pid, ip=opts.udp_ip, x_per_fpga=opts.x_per_fpga, port=opts.udp_port, payload_len=opts.payload_len, timestamp_rnd=opts.timestamp_rounding, verbose=opts.verbose, store_in_cache=opts.cacheit)
    c.start()
